...
ExperimentsDir='/data/neurodev/NTR/Experiments/';
%ExperimentsDir='G:\fMRI\gradCPT\Experiments\';
ExpName='/NTR/';
fmriprep_table_name=['May_11'];
try
...
% 91, 109, 91 for 2mm voxel
% make if statement for which # to use for different voxel size
if strcmp(VoxelSize, '2mm')
    [fmriprep_table] = ExtractAndCleanBoldTCs(ExperimentsDir,fmriprep_table,'Resample',[91,109,91],'VoxelSize',VoxelSize,'Overwrite',0);
    [fmriprep_table] = ExtractAnatMasks(ExperimentsDir,fmriprep_table,'Resample',[91,109,91],'VoxelSize',VoxelSize,'Overwrite',0);
else
...
fmriprep_table.Error=cell(height(fmriprep_table),1);
...
if gradCPT==1
    ...
    save(['fmriprep_table',ExpName,fmriprep_table_name],'fmriprep_table');
    [ExperimentsDir,fmriprep_table] = ComputeBehaviorTCs(fmriprep_table,ExperimentsDir);
    save(['fmriprep_table',ExpName,fmriprep_table_name],'fmriprep_table');
end
