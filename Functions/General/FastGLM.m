function [Condition_tMat,DesignMatrix,Condition_bMat] = FastGLM(Y,X,contrasts,batchSize,varargin)
...
if batchSize<numY
    ...
    if ~isempty(contrasts)
        ...
        if skipContrast==0
            [T,B] = FastOLSRegressContrast(Y,X(:,UseRegressors==1),contrasts(UseContrast==1,:));
        else
