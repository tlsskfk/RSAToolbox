function [AnalysisParameters] = Parcellation_LME(varargin)
...
if isempty(fmriprep_table_names)
    ...
    for i = 1:NumGroups
        [ExperimentsDirs{i,1},~,fmriprep_table_names{i,1}] = load_fmriprep_table('ExperimentsDir',ExperimentsDir);
...
for i = 1:NumGroups
    [~,tempTable] = load_fmriprep_table('ExperimentsDir',ExperimentsDirs{i,1},'fmriprep_table_name',fmriprep_table_names{i,1});
    fmriprep_tables{i,1}=tempTable;
    GroupingVar=[GroupingVar;repmat(fmriprep_table_names(i,2),[height(tempTable),1])];
    ComparisonNames=[ComparisonNames,fmriprep_table_names{i,2}];
end
% fmriprep_tables=AnalysisParameters.fmriprep_tables;
GroupingVar=array2table(GroupingVar,'VariableNames',{'GroupingVar'});
fmriprep_table=[fmriprep_tables,GroupingVar];
...
BaseTable=[Use_fmriprep_table(:,{'GroupVar'}),Use_fmriprep_table(:,RFX_Intercepts),Use_fmriprep_table(:,RFX_Slopes),All_MotionStats,All_Variables];
