function [AnalysisParameters] = ComputeSearchlight2SplitVarRCA(fmriprep_table,ExperimentsDir,varargin)
...
[ComputeClusterThresh] = VariableSetter('ComputeClusterThresh',['None'],varargin);
...
if isempty(SearchlightName)
    [filePaths_Searchlight,~,RSMsName] = BIDsDirSearch(ExperimentsDir,fmriprep_table,'SubjectOrRun',SubjectOrRun,'AnalysisType','SplitVarRSMs','AnalysisName',RSMsName,'TitleTextName','Select fMRI input for GLM:');
...
if isempty(AnalysisName)
    RCAName=strrep(RSMsName,'SplitVarRSMs_',['RCA_',fmriprep_table_name,'_']);
    if DefaultName~=1
...
[filePaths_Searchlight] = BIDsDirSearch(ExperimentsDir,fmriprep_table,'SubjectOrRun',SubjectOrRun,'AnalysisType','SplitVarRSMs','AnalysisName',RSMsName,'TitleTextName','Select fMRI input for GLM:');
...
BrainMap=[];
SpltPrefi={'Split1_','Split2_'};
SaveRF=cell(2,3);
for SP=[1,2]
    S=SpltPrefi{1,SP};
    AllRSMs=[];
    for i =1:size(filePaths_Searchlight,1)
        loadData=load(filePaths_Searchlight{i,1},[S,'RSMs']);
        tempRSMs=loadData.([S,'RSMs']);
        if isempty(AllRSMs)
    ...
    SaveRF{SP,1}=groupRF_mean;
    SaveRF{SP,2}=groupRF_t;
    SaveRF{SP,3}=groupRF_z;
    BrainMap=cat(4,BrainMap,groupRF_mean,groupRF_t,groupRF_z);   
    ...
    for dataInd=useIndicies
        ...
        descript1='desc-SplitVarRCA'; %set file description name
        ...
        if SP==1
            Split1_RFMap=reshape(RFMap,mapSize);    
            save(SaveNames{1,1},'Split1_RFMap');
        else
            Split2_RFMap=reshape(RFMap,mapSize); 
            load(SaveNames{1,1},'Split1_RFMap');
            SplitDiff_RFMap=Split2_RFMap-Split1_RFMap;
            save(SaveNames{1,1},'Split1_RFMap','Split2_RFMap','SplitDiff_RFMap','AnalysisParameters');
        end
    ...
    SaveRF{SP,2}=groupRF_t;
    SaveRF{SP,3}=groupRF_z;
    Split1_groupRF_mean=SaveRF{1,1};
    Split1_groupRF_t=SaveRF{1,2};
    Split1_groupRF_z=SaveRF{1,3};

    Split2_groupRF_mean=SaveRF{2,1};
    Split2_groupRF_t=SaveRF{2,2};
    Split2_groupRF_z=SaveRF{2,3};
...
[SplitDiff_groupRF_t,SplitDiff_groupRF_z,SplitDiff_groupRF_mean]=getTval(atanh(RFMaps),4);
SplitDiff_groupRF_t(isinf(SplitDiff_groupRF_t))=nan;
SplitDiff_groupRF_z(isinf(SplitDiff_groupRF_z))=nan;
SplitDiff_groupRF_mean(isinf(SplitDiff_groupRF_mean))=nan;
BrainMap=cat(4,BrainMap,SplitDiff_groupRF_mean,SplitDiff_groupRF_t,SplitDiff_groupRF_z);
SaveBrik_3mmMNI(BrainMap,{'Split1_group_mean','Split1_group_t','Split1_group_z','Split2_group_mean','Split2_group_t','Split2_group_z','SplitDiff_group_mean','SplitDiff_group_t','SplitDiff_group_z'},[GroupBrainMapsDir,'SplitVar_groupRF_SearchlightMaps']);
save([GroupAnalysisDir,'SplitVargroupRefRF_SearchlightMaps'],'Split1_groupRF_mean','Split1_groupRF_t','Split1_groupRF_z','Split2_groupRF_mean','Split2_groupRF_t','Split2_groupRF_z','SplitDiff_groupRF_mean','SplitDiff_groupRF_t','SplitDiff_groupRF_z','AnalysisParameters');    
