...
if any(cellfun(@isempty,filePaths_beh_events)==0)
    ...
    EventNames=uiNameSelect(EventNamesAll,'Select events to control for');
    TimeCourseNames=uiNameSelect(EventNamesAll,'Select timecourses to control for');
...
for dataInd=useIndicies
    ...
    for repNum=1:numReps
        AllRSMs=cell(numSubs,numParcels);
        all_rsm_mask=cell(numSubs,numParcels);
        ...
        try
            parfor subNum=1:numSubs  
                ...
                [Condition_tVals,outbrain_mask,~,CondReg_CorrMat] = GetActivationPatterns(...
                ...
                if Compute_CSF_RSM == 1 && ~isempty(Use_csfMask)
                    ParcellationVector=Use_csfMask(outbrain_mask~=0); 
                ...
                if Compute_WM_RSM == 1 && ~isempty(Use_wmMask)
                    ParcellationVector=Use_wmMask(outbrain_mask~=0); 
                ...
                for i = 1:numParcels
                    if strcmpi(ParcelNames{i,1},'Searchlight')
                       % brainVals=Condition_tVals;
                        ...
                        UseMask=AllParcels{i,1};
                        UseLabels=AllParcels{i,2};
                        if ~ismember(brainSize,size(UseMask),'rows')
                            UseMask=imresize3(UseMask,brainSize,'nearest');
                        end    
                        UseLabels=UseLabels(:);
                        gmVector=Use_gmMask(outbrain_mask~=0);
                        ParcellationVector=UseMask(outbrain_mask~=0).*gmVector;  
                        if subNum == 1                       
                            all_rsm_mask{subNum,i}=single(outbrain_mask~=0).*UseMask.*Use_gmMask;
                        end
                        [ tempRSMs ] = ParcellationRSMs( Condition_tVals,ParcellationVector,SimType);
                        ...
                        if ~isempty(ConfoundRSMs)
                            AllRSMs{subNum,i} = ComputeCleanRSMs(tempRSMs,ConfoundRSMs);
                        else
                            AllRSMs{subNum,i}=mat2uppertriuvectormat(tempRSMs);
                        end
        ...
        for i = 1:numParcels
            ...
            if repNum == 1
                save(SaveNames{i,1},'RSMs','rsm_mask','AnalysisParams','subsample_data');
            else
