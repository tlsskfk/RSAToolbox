function [RC,RF,IF,RCs,RCx,RFs,RFx,rsm_mask] = GetParcellationSplitRCA_2GLM(Events,ConfoundEvents,TimeCourses,ConfoundTCs,boldTCs,boldMasks,ConditionNames,Parcels,TrialNum,NumReps,varargin)
...
parfor rep = 1:NumReps
    ...
    if SplitAcrossRun==1
        [SplitConditions,~,~] = ComputeSplitConditions(EventsMat,ConditionNames,'SplitVar',rand(size(EventsMat,1),1),'TrialNum',TrialMat,'RandomSplit',1);
        ...
        for run=1:numRuns
            [SplitConditions,~,~] = ComputeSplitConditions(Events{run,1},ConditionNames,'SplitVar',rand(size(Events{run,1},1),1),'TrialNum',TrialNum{run,1},'RandomSplit',1);
            SplitConditions1=SplitConditions(:,1:(size(SplitConditions,2)/2));
            SplitConditions2=SplitConditions(:,(size(SplitConditions,2)/2)+1:end); 
            Split1_Events=single(nansum(SplitConditions1,2)>0); 
            Split2_Events=single(nansum(SplitConditions2,2)>0);                       
    ...
    [Condition_tVals1,~,~,~,DesignMatrix1] = GetActivationPatterns(...
        ...
        'UseStat',UseStat);  
    [Condition_tVals2,brain_mask,~,~,DesignMatrix2] = GetActivationPatterns(...
        ...
        'ResampleToFit',ResampleToFit,...
        'UseStat',UseStat); 
    Condition_tVals=[Condition_tVals1,Condition_tVals2];
    if isempty(gmMask)
    ...
    if Compute_CondReg_CorrMat==1
        CondReg_CorrMat=corrcoef([table2array(DesignMatrix1(:,ConditionNames)),table2array(DesignMatrix2(:,ConditionNames))]);
