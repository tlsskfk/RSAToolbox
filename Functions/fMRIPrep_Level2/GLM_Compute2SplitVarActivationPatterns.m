function [AnalysisParameters] = GLM_Compute2SplitVarActivationPatterns(fmriprep_table,ExperimentsDir,varargin)
...
[numGLM] = VariableSetter('numGLM',[],varargin);
...
if isempty(AnalysisParameters)
    ...
    if strcmpi(LoadParams,'Yes')
        try
            [filePaths_glm,~,glmName] = BIDsDirSearch(ExperimentsDir,fmriprep_table,'AnalysisType','ActivationPattern2SplitVar','TitleTextName','Select Analysis Parameters:');
...
if ~isempty(AnalysisParameters)
    ...
    numGLM=AnalysisParameters.numGLM;
...
AnalysisType='ActivationPattern2SplitVar';
...
if isempty(SplitVar)
    if isempty(EventNames) || isempty(TimeCourseNames)
        for aNum=1:size(filePaths_beh_events,1)
            if ~isempty(filePaths_beh_events{aNum,1})            
                load(filePaths_beh_events{aNum,1},'beh_events');            
                EventNamesAll=unique([EventNamesAll;beh_events.Properties.VariableNames(:)]);
...
if SplitAcrossRun==1
	SplitTypeAffix='_xRun';
else
	SplitTypeAffix='_inRun';    
end
numGLMAffix='';

    if isempty(numGLM)
        SingleSelect=1; %Allows only a single value to be selected.
        [numGLM] = uiNameSelect({'One','Two'},'Number of GLMs?',SingleSelect);
        if strcmpi(numGLM,'One')
            numGLM=1;
...
SplitVarAffix=['_SplitBy',SplitVar];
AnalysisParameters.SplitAcrossRun=SplitAcrossRun;
AnalysisParameters.numGLM=numGLM;
AnalysisParameters.SplitVar=SplitVar;
AnalysisParameters.ConditionNames=ConditionNames;
tempName=strrep(['ActivationPattern_',afniSuffix,SplitVarAffix,SplitTypeAffix,numGLMAffix],'__','_');
if UseDefaultName == 0    
...
for dataInd=useIndicies
    ...
    %% Run analysis here!!
    try
        [~,brain_mask,SplitData,DesignMatrix,AfniInfo,Split1_tVals,Split2_tVals,Split1_bVals,Split2_bVals] = GetParcellation2SplitVarRCA(...
            Use_Events,Use_ConfoundEvents,Use_TimeCourses,Use_ConfoundTCsByRun,...
            BaseTC,brain_mask,ConditionNames,[],TrialNum,UseSplitVarTC,...
            'numGLM',numGLM,...
            'UseAfni',UseAfni,...
    ...
    save(SaveNames,'SplitData','brain_mask','DesignMatrix','AfniInfo','Split1_tVals','Split2_tVals','Split1_bVals','Split2_bVals','AnalysisParameters');     
    toc
end
end