function [AnalysisParameters] = ComputeParcellationRCA(fmriprep_table,ExperimentsDir,varargin)
...
SubSampleRSM = VariableSetter( 'SubSampleRSM',[],varargin);
SubSampleSize = VariableSetter( 'SubSampleSize',[],varargin);
SubSampleVar = VariableSetter( 'SubSampleVar',[],varargin);
...
if ~isempty(AnalysisParameters)
    ...
    SubSampleRSM = AnalysisParameters.SubSampleRSM;
    SubSampleSize = AnalysisParameters.SubSampleRSM;    
    SubSampleVar = AnalysisParameters.SubSampleVar;
else
...
if isempty(SubSampleRSM)
    SingleSelect=1; %Allows only a single value to be selected.
    [SubSampleRSM] = uiNameSelect({'Yes','No'},'Sub sample RSM?:',SingleSelect);
    if strcmpi(SubSampleRSM,'Yes')
        SubSampleRSM=1;
    else
        SubSampleRSM=0;
    end
...
if SubSampleRSM==1 
    if isempty(SubSampleSize)
        SubSampleSize=uiEnterName('','Enter subsample size');
        SubSampleN=str2num(SubSampleSize);
    else
        SubSampleSize=num2str(SubSampleN);
    end
    SubSampleSuffix=['_N',SubSampleSize];
    [~,StudyNames]=getFolderAndFileNames('SubSampleVecs/');
    StudyName=uiNameSelect(StudyNames,'Select split var file',1);
    SubSampleVars=load(['SubSampleVecs/',StudyName{1,1}]);
    StudyName=strrepCell(StudyName,'.mat','');
    SubSampleVars=SubSampleVars.(StudyName{1,1});
    SubSampleVarNames=[{'Random'},SubSampleVars.Properties.VariableNames];
    SubSampleVarName=uiNameSelect(SubSampleVarNames,'Select subsample variable.',1);
    if ~strcmpi(SubSampleVarName,'Random')
        SubSampleVar=SubSampleVars.(SubSampleVarName);
    else
        SubSampleVar=1;
    end
else
...
AnalysisParameters.SubSampleRSM=SubSampleRSM;
AnalysisParameters.SubSampleSize=SubSampleSize;
AnalysisParameters.SubSampleVar=SubSampleVar;
...
newDesc='desc-RCA'; %set file description name
...
for parcelNum=1:numParcels
    ...
    if SubSampleRSM==0
        ...
        if ndims(AllRSMs) == 3
            [ OutVars ] = FullRCA( AllRSMs,'vertIn',1,...
            ...
            [ OutVars ] = FullRCA( AllRSMs,'vertIn',0,...
                'DownSample',DownSample,...
                'DownSampleN',DownSampleN,...
                'DownSampleReps',DownSampleReps,...
                'RunPermute',RunPermute,...
        ...
        RFMats=single(OutVars.RF);
        RCMats=single(OutVars.RCMat); 
    else
        ...
        if SubSampleVar==1
            SubRSMVertSize = (SubSampleN^2-SubSampleN)/2;
            SubSampleVar=zeros(RSMSize,1);
            SubSampleVar(1:SubSampleN,1)=1;
        end
        RFMats=[];
        RCMats=[];
        ALLRF=zeros(size(AllRSMs,2),size(AllRSMs,3),100,'single');
        ALLRC=zeros(size(AllRSMs,2),size(AllRSMs,2),size(AllRSMs,3),100,'single');
        for rep = 1:100
            tempSubSample=SubSampleVar(randperm(size(SubSampleVar,1)));
...
%             end
        ...
        RCA_RF=nanmean(ALLRF,3);
        RCA_RC=nanmean(ALLRC,4);
        STD_RF=nanstd(ALLRF,0,3);
        STD_RC=nanstd(ALLRC,0,4);
    end
    for loadInd = 1:length(UseInd)
        loadNum=UseInd(loadInd,1);
        RCA_RF=RFMats(:,loadInd);
        try
            RCA_RF=array2table(RCA_RF','VariableNames',UseLabels);
        catch
        ...
        RCA_RC=RCMats(:,:,loadInd);
        RCA_RC=array2table(RCA_RC,'VariableNames',UseLabels,'RowNames',UseLabels);
        ...
        if OutputPermDists == 1
            ...
            save(SaveName,'RCA_RF','RCA_RC','RCA_RF_Z','RCA_RC_Z','IndvRCA_RC','IndvRCA_RC_Z','RCA_RC_Degree','RCA_RC_WholeCM','AnalysisParameters');
        end
    end
    ...
    save([GroupAnalysisDir,'ThirdOrderRSMs_RF'],'ssRSM_RF','parcelRSM_RF','ssRSM_RF_scaled','parcelRSM_RF_scaled','UseInd','UseTable');
    save([GroupAnalysisDir,'AnalysisParameters'],'AnalysisParameters','UseInd','UseTable');
    save([GroupAnalysisDir,'Group_RF'],'Group_RF');
