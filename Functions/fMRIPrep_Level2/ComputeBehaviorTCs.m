...
if isempty(PullParams)
    [PullParams] = uiNameSelect({'Yes','No'},'Pull exisiting parameters when availible?',SingleSelect);
...
if PullParams==0
    for dataInd=useIndicies
        ...
        SetParamStimVars=unique([SetParamStimVars;StimVars]);
        SetParamStimVars(ismember(SetParamStimVars,''),:)=[];
        Responses=[];
        ...
        if IncludeResponse==1
            ...
            SetParamResponseVars=unique([SetParamResponseVars;Responses]);
            SetParamResponseVars(ismember(SetParamResponseVars,''),:)=[];
        end    
    end
    try
        [ ~,~,UseParametersIni ] = ExtractEvents_General(SetParamStimVars,[],...
            'Responses',SetParamResponseVars,...
            'TrialDuration',[],...
            'SetUseParameters',1);
    catch
        UseParameters=[];
...
while redo==1
    ...
    for dataInd=useIndicies
        ...
        elseif exist(SaveNames{1,1},'file')~=0 && Overwrite==1 
            ...
            if PullParams==1    
                load(SaveNames{1,1},'UseParametersIni')
            end
        end
        ...
        try
            [ beh_clean,beh_events,UseParameters ] = ExtractEvents_General(StimVars,Onsets,...
                ...
                'UseParameters',UseParametersIni,...
                'mseq',mseq,...
                'fmriDur',fmriDur);
         catch
            ...
            save(SaveNames{1,1},'beh_events','UseParameters','UseParametersIni'); 
            save(SaveNames{2,1},'beh_clean','UseParameters','UseParametersIni'); 
